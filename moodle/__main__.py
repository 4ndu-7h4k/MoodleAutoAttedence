...
def check():
    ...
    log.info(f"{WARNING}{now_asia.strftime(format)} : Attendence checking for all Courses{ENDC}\n\n")
    k = 1
    while k<=4:
        ...
        if m.tcount == m.tcourse:
            ...
            now_utc = datetime.now(timezone('UTC'))
            now_asia = now_utc.astimezone(timezone('Asia/Kolkata'))
            log.info(f"{now_asia.strftime(format)} : All classes don't have attendence now!")
            if k == 4:

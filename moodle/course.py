...
class course(moodle):
    def get_course(self):
        payload = {'perpage':'50'}
        r = self.session.get(f'http://moodle.mec.ac.in/course/index.php?categoryid={SEMID}',params=payload)
        soup = BeautifulSoup(r.text,"html.parser")
        soup = soup.find("div",attrs={"class":"course_category_tree clearfix"})
        course_url_list = [url["href"] for url in soup.select('a[href^="http://moodle.mec.ac.in/course/view.php?id="]')]

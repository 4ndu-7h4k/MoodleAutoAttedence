...
class attendence(course):
    ...
    def mark_attedence(self,atturl,name):
        ...
        soup = BeautifulSoup(r.text,"html.parser")
        url = soup.select('a[href^="http://moodle.mec.ac.in/mod/attendance/attendance.php?sessid="]')
        if url and url[0].text == "Submit attendance": 
            ...
            if r.url == atturl:
                ...
                self.tcount -=1
                log.info(f"{CYAN}{self.get_course_name(atturl)}  -> Attedence marked successfully  at {now_asia.strftime(format)}{ENDC}")
            ...
            now_utc = datetime.now(timezone('UTC'))
            now_asia = now_utc.astimezone(timezone('Asia/Kolkata'))
            log.info(f"{name} --> course don't have attedence at ({now_asia.strftime(format)})")
